<script setup>
   import {   ref,  computed,  onMounted,  onBeforeUnmount,  reactive} from 'vue';
    ...
    const props = defineProps(
    {
        ...
        url:{default:'roles/filtered',type: String},
        ...
        placeholder:{default:'Select role',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'role_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'role_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
 ...
 onBeforeUnmount(() => {
        $('.role_'+props.index).select2('destroy');
    ...
    const intialiseSelect = ()=>{
        ...
        var itemComponent = $('.role_'+props.index);
        ...
        .on('change', function () {
            var item = $('.role_'+ props.index).val();
        ...
        .on('select2:select', function (e) {
            ...
            if( props.selected_data != {}){
               emit('updateState',item,props.index,props.selected_data);
            }
        })
        ...
        if(typeof(props.index) !="undefined"){
            $('.role_'+props.index).val(props.selected).change();
        }
    }
