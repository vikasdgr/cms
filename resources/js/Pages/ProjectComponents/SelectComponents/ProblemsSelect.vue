<script setup>
   import {   ref,  computed, onMounted, onBeforeUnmount,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateProblem','update:modelValue']);
    ...
    const props = defineProps(
        {
            ...
            url:{default:'problems/filtered',type: String},
            ...
            placeholder:{default:'Select Problem',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'problem_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opaproblem-50 rounded-md shadow-sm mt-1 block w-full'
                            :'problem_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opaproblem-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onBeforeUnmount(() => {
        $('.problem_'+props.index).select2('destroy');
        // console.log( $('.problem_'+props.index));
    });
    const intialiseSelect = ()=>{
        var itemComponent = $('.problem_'+props.index);
        ...
        .on('change', function () {
            ...
            var item = $('.problem_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateProblem',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateProblem',item,props.index,selected_data.value);
            }
        })
        ...
        if(typeof(props.index) !="undefined"){
            $('.problem_'+props.index).val(props.selected).change();
        }
    }
