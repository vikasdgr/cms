<script setup>
   import {   ref,  computed, onMounted, onBeforeUnmount,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateMachineType','update:modelValue']);
    ...
    const props = defineProps(
        {
            ...
            url:{default:'machine-types/filtered',type: String},
            ...
            placeholder:{default:'Select Machine Type',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'machine-type_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'machine-type_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onBeforeUnmount(() => {
        $('.machine-type_'+props.index).select2('destroy');
        // console.log( $('.machine-type_'+props.index));
    });
    const intialiseSelect = ()=>{
        var itemComponent = $('.machine-type_'+props.index);
        ...
        .on('change', function () {
            ...
            var item = $('.machine-type_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateMachineType',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateMachineType',item,props.index,selected_data.value);
            }
        })
        ...
        if(typeof(props.index) !="undefined"){
            $('.machine-type_'+props.index).val(props.selected).change();
        }
    }
