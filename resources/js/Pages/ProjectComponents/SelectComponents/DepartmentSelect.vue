<script setup>
   import {   ref,  computed, onMounted, onBeforeUnmount,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateDepartment','update:modelValue']);
    ...
    const props = defineProps(
        {
            ...
            url:{default:'departments/filtered',type: String},
            ...
            placeholder:{default:'Select Department',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'department_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'department_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onBeforeUnmount(() => {
        $('.department_'+props.index).select2('destroy');
        // console.log( $('.department_'+props.index));
    });
    const intialiseSelect = ()=>{
        var itemComponent = $('.department_'+props.index);
        ...
        .on('change', function () {
            ...
            var item = $('.department_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateDepartment',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateDepartment',item,props.index,selected_data.value);
            }
        })
        ...
        if(typeof(props.index) !="undefined"){
            $('.department_'+props.index).val(props.selected).change();
        }
    }
