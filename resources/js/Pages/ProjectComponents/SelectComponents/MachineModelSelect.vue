<script setup>
   import {   ref,  computed, onMounted, onBeforeUnmount,  reactive} from 'vue';
    ...
    const emit = defineEmits(['updateMachineModel','update:modelValue']);
    ...
    const props = defineProps(
        {
            ...
            url:{default:'machine-model/filtered',type: String},
            ...
            placeholder:{default:'Select Machine Model',type:String},
    ...
    const classes = computed(() => {
        return props.error ? 'machine-model_'+props.index+' border-red-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full'
                            :'machine-model_'+props.index+' border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 block w-full';
    });
    ...
    onBeforeUnmount(() => {
        $('.machine-model_'+props.index).select2('destroy');
        // console.log( $('.city_'+props.index));
    });
    const intialiseSelect = ()=>{
        var itemComponent = $('.machine-model_'+props.index);
        ...
        .on('change', function () {
            ...
            var item = $('.machine-model_'+ props.index).val();
            if(props.multiple == false){
               emit('update:modelValue', item);
            }
            else if(props.multiple == true){
               emit('updateMachineModel',item,props.index);
            }
        })
        .on('select2:select', function (e) {
            ...
            if( selected_data.value != {}){
               emit('updateMachineModel',item,props.index,selected_data.value);
            }
        })
        ...
        if(typeof(props.index) !="undefined"){
            $('.machine-model_'+props.index).val(props.selected).change();
        }
    }
