    ...
    state.table = $('#service_list').DataTable({
        ...
        buttons: [
            'pageLength',
            {
                ...
                filename:function(){
                    return 'Housekeeping Machines Cases Report (From Date :' +filter_form.from_date+' To Date :'+filter_form.to_date+') ';
              },
                title: function () {
                    var str = 'Housekeeping Machines Cases Report (From Date :' +filter_form.from_date+' To Date :'+filter_form.to_date+') ';
                    return str;
                },
            },
             {
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value + "/cases-report", // Update URL
            "type": "GET",
             "data":function(c){
                c.report = true;
        ...
        columns: [{
            ...
            },
            {
            ...
            },
             {
                ...
                data: 'status',
                "render": function(data, type, row, meta) {
                   let str = data =='P' ? 'Pending':(data == 'C'?
                   (row.closed_date ? 'Closed <br> ('+row.closed_date+')' :'Closed')
                   :data == 'F'?'Follow-Up Required':'');
                    return str;
                }
            },
        ],
...
<template>
    <AppLayout title="Machine Cases Report ">
        <case-filter :key="state.random" :form="filter_form" @reloadTable="resetForm" v-show="state.formOpen == false">
        </case-filter>
        <div>
            <ListWrapper :title="'Machine Cases Report ( '+filter_form.from_date +' - '+filter_form.to_date+' )' " >
                <template #table>
