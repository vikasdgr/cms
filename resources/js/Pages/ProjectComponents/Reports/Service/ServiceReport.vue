    ...
    state.table = $('#service_list').DataTable({
        ...
        buttons: [
            'pageLength',
            {
                ...
                filename:function(){
                    return 'Housekeeping Machines Service Report (From Date :' +filter_form.from_date+' To Date :'+filter_form.to_date+') ';
              },
                title: function () {
                    var str = 'Housekeeping Machines Service Report (From Date :' +filter_form.from_date+' To Date :'+filter_form.to_date+') ';
                    return str;
                },
            },
             {
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value + "/services-report", // Update URL
            "type": "GET",
             "data":function(c){
                c.report = true;
        ...
        columns: [{
            ...
            },
            {
            ...
            },
             {
            ...
            },
               {
                ...
                data: 'case_no',
                   "render": function(data, type, row, meta) {
                    return "<a   data-item-id="+ row.case_id + " class='font-bold cursor-pointer text-sky-600 text-base services' >"+data+"</a>";
                }
            },
              {
                title: 'Brand',
            ...
            },
            {
                ...
                data: 'service_type',
                "render": function(data, type, row, meta) {
                    return data =='M' ?'Maintenance':(data =='R'?'Repair':'Installation');
                }
            },
              {
                title: 'No. of Issues',
             ...
             {
                title: 'Status',
                data: 'closed_service_no',
                "render": function(data, type, row, meta) {
                    return  row.status == 'C' ?'Closed':(data ? 'Closed in Follow Up Service No. '+data: (row.status =='F' ? 'Follow Up Required':''));
                }
            },
        ],
     ...
     $('thead > tr> th:nth-child(6)').css({ 'min-width': '150px' });
     $('thead > tr> th:nth-child(7)').css({ 'min-width': '100px' });
     $('thead > tr> th:last-child').css({ 'min-width': '120px' });
};
...
<template>
    <AppLayout title="Machine Report ">
        ...
        <div>
            ...
            <ListWrapper :title="'Service  Report ( '+filter_form.from_date +' - '+filter_form.to_date+' )' ">
                <template #table>
