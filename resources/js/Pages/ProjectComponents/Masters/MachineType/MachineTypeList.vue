...
import MachineTypeForm from '@/Pages/ProjectComponents/Masters/MachineType/MachineTypeForm.vue'; // Update import path
...
const setTable = () => {
    state.table = $('#machine_type_list').DataTable({
        "processing": true,
        "serverSide": true,
        "searchDelay": 700,
        "ordering": true,
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value+"/machine-types-list", // Update URL
            "type": "GET",
        },
        ...
        "drawCallback": function (settings) {
            $(".edit-item").on('click', function (e) {
                state.formOpen = false;
                editMachineType(e.target.dataset.itemId);
            });
...
<template>
    <AppLayout title="Machine Types">
        ...
        <div>
            <MachineTypeForm v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id"
            >
            </MachineTypeForm>
            <ListWrapper title="Machine Types List">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false">
                        <i class="fa-solid fa-plus mr-2"></i> New Machine Type
                    </ButtonComp>
                </template>
                <template #table>
                    <table id="machine_type_list" width="100%" class="row-border stripe">
                    </table>
                </template>
            </ListWrapper>
        </div>
