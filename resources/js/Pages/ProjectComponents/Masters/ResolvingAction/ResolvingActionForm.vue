...
const data = reactive({ create_url: 'resolving-actions' }); // Update URL
const pageTitle = computed(() =>
  props.form_id > 0 ? 'Update Resolving Action' : 'Add Resolving Action'
);

const emit = defineEmits(['resetForm']);
onMounted(() => {
    if (props.form_id > 0) {
        getResolvingAction(); // Update function name
    }
});
...
const getResolvingAction = () => {
    axios .get(base_url.value + '/resolving-actions/' + props.form_id + '/edit') // Update URL
    .then(function (response) {
        if (response.data.success) {
            let resolvingAction = response.data.resolvingAction; // Update variable name
            form.name = resolvingAction.name; // Update variable name
            form.description = resolvingAction.description; // Update variable name
