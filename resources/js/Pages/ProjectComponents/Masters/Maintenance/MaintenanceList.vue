...
import MaintenanceForm from '@/Pages/ProjectComponents/Masters/Maintenance/MaintenanceForm.vue'; // Update import path
...
const setTable = () => {
  state.table = $('#maintenance_list').DataTable({
    "processing": true,
    "serverSide": true,
    "searchDelay": 700,
    "ordering": true,
    ...
    pageLength: 10,
    "ajax": {
      "url": base_url.value + "/maintenances-list", // Update URL
      "type": "GET",
    },
    ...
    "drawCallback": function (settings) {
      $(".edit-item").on('click', function (e) {
        state.formOpen = false;
        editMaintenance(e.target.dataset.itemId);
      });
...
<template>
  <AppLayout title="Maintenance List">
    <template #header>
    ...
    <div>
      ...
      ></MaintenanceForm>
      <ListWrapper title="Maintenance List">
        <template #button>
          <ButtonComp
            @buttonClicked="state.formOpen = true"
            type="new"
            v-if="state.formOpen == false"
          >
            <i class="fa-solid fa-plus mr-2"></i> New Maintenance
          </ButtonComp>
        </template>
        <template #table>
          <table id="maintenance_list" width="100%" class="row-border stripe">
          </table>
        </template>
      </ListWrapper>
    </div>
