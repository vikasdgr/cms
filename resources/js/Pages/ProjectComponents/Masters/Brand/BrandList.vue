...
import BrandForm from '@/Pages/ProjectComponents/Masters/Brand/BrandForm.vue'; // Update import path
...
const setTable = () => {
    state.table = $('#brands_list').DataTable({
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value + "/brands-list", // Update URL
            "type": "GET",
        },
...
<template>
    <AppLayout title="Brands">
        ...
        <div>
            <BrandForm v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id">
            </BrandForm>
            <ListWrapper title="Brands List">
                ...
                <template #table>
                    <table id="brands_list" width="100%" class="row-border stripe">
                    </table>
                </template>
            </ListWrapper>
        </div>
