...
const data = reactive({ create_url: 'brands' }); // Update URL
const pageTitle = computed(() =>
    props.form_id > 0 ? 'Update Brand' : 'Add Brand'
);

const emit = defineEmits(['resetForm']);
onMounted(() => {
    if (props.form_id > 0) {
        getBrand(); // Update function name
    }
});
const submitForm = () => {
...
const getBrand = () => {
    axios
        .get(base_url.value + '/brands/' + props.form_id + '/edit') // Update URL
        .then(function (response) {
            if (response.data.success) {
                let brand = response.data.brand; // Update variable name
                form.name = brand.name; // Update variable name
            }
