...
import ProblemForm from '@/Pages/ProjectComponents/Masters/Problem/ProblemForm.vue'; // Update import path
...
const setTable = () => {
    state.table = $('#problems_list').DataTable({
        "processing": true,
        "serverSide": true,
        "searchDelay": 700,
        "ordering": true,
        ...
        pageLength: 10,
        "ajax": {
            "url": base_url.value + "/problems-list", // Update URL
            "type": "GET",
        },
        ...
        "drawCallback": function(settings) {
            $(".edit-item").on('click', function(e) {
                state.formOpen = false;
                editProblem(e.target.dataset.itemId);
            });
        }
    });
};
...
<template>
    <AppLayout title="Malfunctions">
        ...
        <div>
            <ProblemForm v-if="state.formOpen" @resetForm="resetForm" :form_id="state.form_id">
            </ProblemForm>
            <ListWrapper title="Malfunctions List">
                <template #button>
                    <ButtonComp @buttonClicked="state.formOpen = true" type="new" v-if="state.formOpen == false">
                        <i class="fa-solid fa-plus mr-2"></i> New Problem
                    </ButtonComp>
                </template>
                <template #table>
                    <table id="problems_list" width="100%" class="row-border stripe">
                    </table>
                </template>
            </ListWrapper>
        </div>
