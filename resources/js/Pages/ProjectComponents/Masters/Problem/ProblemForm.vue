...
const data = reactive({ create_url: 'problems' }); // Update URL
const pageTitle = computed(() =>
  props.form_id > 0 ? 'Update Problem' : 'Add Problem'
);

const emit = defineEmits(['resetForm']);
onMounted(() => {
  if (props.form_id > 0) {
    getProblem(); // Update function name
  }
});
const submitForm = () => {
...
const getProblem = () => {
  axios
    .get(base_url.value + '/problems/' + props.form_id + '/edit') // Update URL
    .then(function (response) {
      if (response.data.success) {
        let problem = response.data.problem; // Update variable name
        form.name = problem.name; // Update variable name
        form.description = problem.description; // Update variable name
