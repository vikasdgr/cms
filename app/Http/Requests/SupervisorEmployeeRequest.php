...
use App\Models\Supervisor\SupervisorEmpDependent;
...
class SupervisorEmployeeRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        ...
        $rules =  [
            ...
            'supervisor_dependents.*.dep_age'=>'nullable|numeric',
            'supervisor_dependents.*.dep_aadhar_no'=>'nullable|numeric|min:12|max:12',
    ...
    public function save(){
        ...
        $employee = SupervisorEmployee::findOrNew($this->form_id);
        ...
        // }
        $employee_old_det_ids = $employee->supervisor_dependents()->pluck('id')->toArray();
        $supervisor_dependents = new Collection();
        foreach($this->supervisor_dependents as $dependent){
            $dep = SupervisorEmpDependent::findOrNew($dependent['id']);
            $dep->fill($dependent);
            $supervisor_dependents->add($dep);
        }

        $emp_new_det_ids = $supervisor_dependents->pluck('id')->toArray();
        ...
        $resources = new Collection();
              foreach($this->resources as $resource){
                  ...
                  $resource_d->resourceable_type = SupervisorEmployee::class;
        ...
        DB::beginTransaction();
            $employee->save();
            $employee->supervisor_dependents()->saveMany($supervisor_dependents);
            SupervisorEmpDependent::whereIn('id',$detach);
